...
ENV = 'CartPole-v1'  # 任务名
GAMMA = 0.99  # 时间折扣率
MAX_STEPS = 500  # 1次试验中的最大step数
NUM_EPISODES = 1000  # 最大回合次数
BATCH_SIZE = 32 # batch size
CAPACITY = 10000 # 经验存储容量
SUCCESS_STEPS = 300 # 1次实验中超过则奖励分数即完成任务的step数
End_Condition = 10 # 训练终止条件：终止循环所需连续完成任务数
...
def display_frames_as_gif(frames):
    ...
    anim.save('movie_cartpole_DQN.gif',writer='pillow')
    plt.show()
...
class Environment:
    ...
    def run(self):
        ...
        complete_episodes = 0  # 持续站立SUCCESS_STEPS步，或更多的实验次数
        ...
        for episode in range(NUM_EPISODES):
            ...
            for step in range(MAX_STEPS):
                ...
                if done:  # step超过最大步数，或者倾斜超过某个角度，done为True
            ...
            if complete_episodes >= End_Condition:
                print('连续{}轮成功'.format(End_Condition))
